...
const initialHistory: HistoryInfo[] = [
  {
    id: 1,
    title: '새 공지',
...
const HistoryContentsComponent = () => {
  const [history, setHistory] = useState<HistoryInfo[]>(initialHistory);

  const handleDelete = (id: number) => {
    setHistory(history.filter((notif) => notif.id !== id));
  };

  const markAsRead = (id: number) => {
    setHistory(history.map((notif) => (notif.id === id ? { ...notif, isRead: true } : notif)));
  };

  return (
    <div className={`${styles.start} ${styles.history_contents}`}>
      {/* 알림 없는 경우 */}
      {history.length === 0 ? (
        ...
        <ul className={styles.info_list}>
          {history.map(({ id, title, message, time, isRead, hasLink }) => (
            <li
              key={id}
