 ...
 */
const SingleAttachmentFormFieldComponent = forwardRef<
...
>(
  (
    ...
    useEffect(() => {
      if (uuidType === 'group') {
        if (value && value !== groupUuid) fetchGroupInfo(value);
        return;
      }
    ...
    useEffect(() => {
      ...
      if (fileUuid && fileUuid !== first(uploadFileUuid)) {
        fetchFileInfo([fileUuid]);
      }
    }, [fileUuid]);
...
// 폼 필드 컴포넌트 내보내기
export const SingleAttachmentFormField = SingleAttachmentFormFieldComponent;
