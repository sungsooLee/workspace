...
import { queryOptions } from '@entities/round/service/round.queries';
...
// type RoundListComponentProps = {

// };

/**
 * NLP_BO_LMS_0031 : 차수 목록
 * @returns
 */
const RoundListComponent = () => {
  ...
  _global.linkClick = (payload: any) => {
    router.navigate({
      to: '/learning-operate/round/round-detail',
      state: {
  ...
  useEffect(() => {
    const columns = [
      ...
      columnHelper.accessor('courseSequenceCd', {
        header: t('차수코드'),
        cell: (info) => info.getValue(),
        enableGrouping: false,
        size: 80,
  ...
  const onCopyRow = async () => {
    ...
    const lastSeq = selectedRows.reduce((max, row) => Math.max(max, row.sequence), 0);
    ...
    for (const row of selectedRows) {
      for (let i = 0; i < inputCopy; i++) {
        ...
        clonedRows.push({
          ...
          courseSequenceCd: row.courseSequenceCd,
...
export const RoundList = RoundListComponent;
