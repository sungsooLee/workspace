...
export const useCurriculumActions = ({
  ...
  const handleFormSubmit = useCallback((data: any, formState: any) => {
  ...
  }, [api, clearAllValidators, curriculumDetail, curriculumId, expandParentNodes, handleNodeSelect, loginUser, onCurriculumCreated, onFormChange]);

  const handleLessonSubmit = useCallback((data: any, formState: any, parentNode: TreeNode | null) => {
  ...
  const handleDeleteNode = useCallback(({
    ...
    if (
      ...
      const moduleId =
        node.type === MAPPING_CURRICULUM_TYPE.LESSON ? node.moduleId : api.extractIdFromNodeId(node.id);
  ...
  const handleTreeAction = useCallback((event: any) => {
          ...
          if (payload.fromMappingType === 'LESSON') {
            ...
            if (updatedLessonNode) {
              ...
              if (newParentNode?.type === MAPPING_CURRICULUM_TYPE.CURRICULUM) {
                ...
                newModuleId = newParentNode.data?.moduleId || api.extractIdFromNodeId(newParentNode.id);
              }
