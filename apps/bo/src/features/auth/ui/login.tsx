...
import { getSavedUserid, LoginErrorAlert, TenantRoleModal, useAuthSignin } from '@features/auth';
...
function LoginComponent({ Route }: any) {
    ...
    <form id="myForm" onSubmit={onSubmit(handleOnSubmit)}>
      <div className={`${styles.start} ${styles.auth_wrap} ${styles.login}`}>
        <div className={authStyles.auth_box}>
          <ContentsRow>
            <FormRow2
              label={t('LABEL.form.input.idEmail')}
              provider={provider}
              name={'username'}
              validation={{ required: true, format: 'email' }}
              element={
                <Input
                  placeholder={t('아이디/이메일을 입력하세요')}
                  onKeyDown={(e) => {
          ...
          <ContentsRow className={formStyles.no_line}>
            <FormRow2
              label={t('LABEL.form.input.password')}
              provider={provider}
              name={'password'}
              // TODO format 패스워드로 변경 필요
              // validation={{ required: true, format: 'password' }}
              validation={{ required: true, format: 'text' }}
              element={
                <Input
          ...
          <ContentsRow className={cn(formStyles.no_line, styles.login_info)}>
            <FormRow2
              ...
              element={
                <CheckBoxFormField
                  checkConfig={{ label: t('LABEL.common.saveAccount') }}
                  className={styles.id_save}
                  size="md"
                />
              }
            />
