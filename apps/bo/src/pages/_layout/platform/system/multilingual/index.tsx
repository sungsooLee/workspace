...
function RouteComponent() {
  ...
  const { getLanguageName } = useLanguageMap();
  // const keyTypeCode = useWatch({ control, name: 'keyTypeCode' });
  const keyTypeCode = watch('keyTypeCode');
  const [isSubmitting, setIsSubmitting] = useState(false);
        ...
        <MainContents>
          <SearchBoxForm onSearch={onSubmit(handleOnSearch)}>
            <ContentsRow>
              ...
              />
              <FormRow2
                key={`targetLocale-${keyTypeCode}`}
                provider={sProvider}
                name={'targetLocale'}
                label={t('LABEL.platform.system.multilingual.translationLanguage')}
                element={
                  <DropdownFormField
                    optionsConfig={{
                      // options: [{ value: '', label: 'LABEL.form.label.select' }],
                      ...
                      transformOptions: (options: SelectOption[]) => {
                        const baseOption = [{ value: '', label: 'LABEL.form.label.select' }];
                        setValue('targetLocale', '');
                        if (keyTypeCode === 'HRD_CENTER_MENU') {
                          const tmp = options.filter(
                            (option: SelectOption) => option.value === 'EN',
                          );
                          return [...baseOption, ...tmp];
                        }
