...
export function NxWelcome({ title }: { title: string }) {
  ...
  const TEST_QUERY_KEY = 'LOCAL-TEST-KEY';

  const { data, refetch, isError, isFetching } = useQuery<any>({
    queryKey: [TEST_QUERY_KEY],
    queryFn: async () => await httpService.get('http://localhost:3333/api/get/nodejs-api'),
    enabled: true,
  });
