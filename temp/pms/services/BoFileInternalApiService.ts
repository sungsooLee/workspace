...
import type { com_ever_edu_pms_file_dto_res_SimpleGroupFileInfoListResDto } from '../models/com_ever_edu_pms_file_dto_res_SimpleGroupFileInfoListResDto';
...
export class BoFileInternalApiService {
     ...
     * @returns com_ever_edu_pms_file_dto_res_SimpleGroupFileInfoListResDto OK
     * @throws ApiError
     */
    public static getGroupSimpleFileInfoList(
        groupUuid: string,
    ): CancelablePromise<com_ever_edu_pms_file_dto_res_SimpleGroupFileInfoListResDto> {
        return __request(OpenAPI, {
            method: 'GET',
            url: '/internal/api/v1/file/group/uuid/files',
            query: {
                'groupUuid': groupUuid,
            },
            errors: {
     ...
     * Simple 파일 정보 목록 조회 - Internal API
     * 간략한 파일 정보 목록을 조회한다.<BR>예를 들어 Module-CMS에서 섬네일 파일 처리 시 파일 정보를 조회한다
     * @param groupId 파일그룹 ID
     * @returns com_ever_edu_pms_file_dto_res_GroupInfoResDto OK
     * @throws ApiError
     */
    public static getGroupSimpleFileInfoList1(
        groupId: number,
